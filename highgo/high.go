...
var (
	unmatchedResponses = monitoring.NewInt(nil, "highgo.unmatched_responses")
)

func init() {
	protos.Register("highgo", New)
}
...
func (pgsql *pgsqlPlugin) publishTransaction(t *pgsqlTransaction) {
	...
	logp.Debug("highgo", "发布事务消息, trans_id: %s", t.transId)
