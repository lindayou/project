...
func (p *parser) parse(buf *streambuf.Buffer, conn *redisConnectionData) (bool, bool) {
	snapshot := buf.Snapshot()

	content, iserror, ok, complete := p.dispatch(0, buf, conn)
	if !ok || !complete {
...
func (p *parser) dispatch(depth int, buf *streambuf.Buffer, conn *redisConnectionData) (common.NetString, bool, bool, bool) {
	if buf.Len() == 0 {
	...
	switch buf.Bytes()[0] {
	case '*':
		value, iserror, ok, complete = p.parseArray(depth, buf, conn)
		//value =auth qwe123-=
	case '$':
		value, ok, complete = p.parseString(buf)
		// 截取version  os  arch
		findField(value, conn)
	case ':':
...
func (p *parser) parseArray(depth int, buf *streambuf.Buffer, conn *redisConnectionData) (common.NetString, bool, bool, bool) {
	line, err := buf.UntilCRLF()
	if err != nil {
		if isDebug {
	...
	for i := 0; i < int(count); i++ {
		...
		value, iserror, ok, complete := p.dispatch(depth+1, buf, conn)
		if !ok || !complete {
			if isDebug {
...
func parseInt(line []byte) (int64, error) {
	...
	// TO-DO: is it an error if 'buf.Len() != 0 {}' ?
}
...
func findField(value common.NetString, conn *redisConnectionData) {
	info := strings.Split(string(value), "\r\n")
	for _, item := range info {
		val := strings.ToLower(item)
		if strings.HasPrefix(val, "redis_version:") {

			conn.tcpSession.ServerVersion = val[13:]
		} else if strings.HasPrefix(val, "os:") {

			conn.tcpSession.ServerOS = val[2:]
		} else if strings.HasPrefix(val, "arch_bits:") {

			conn.tcpSession.ServerArch = val[9:]
			break
		}
